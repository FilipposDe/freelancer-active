            ...
            { projects.map( (project, index) => {
                return (
                    ...
                    >
                        ...
                        <p className="client">Rating: { project.clientRating.toFixed( 1 ) } Reviews: { project.clientReviews }</p>
                        <h2>{ project.title }</h2>
                        <p className="desc">{ project.desc }</p>
                        <p className="bids">Bids: <strong>{ project.bids }</strong></p>
                        <p className="budget">
                            Budget: { project.budget } { project.currency },
                        Average Bid: { project.avg ? project.avg.toFixed( 1 ) : "-" } { project.currency }</p>
